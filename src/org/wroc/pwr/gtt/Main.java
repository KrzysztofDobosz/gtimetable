...
public class Main
{
   ...
   public static void main(String[] args)
   {
      ...
      // rozkladami z serwera
      xmlFiles = TTdownloader.unzip(archName, dir); // rozpakowanie archiwum
      // do odpowiedniego katalogu, zwraca liste nazw rozpakowanych plikow
      DBconnector connector = new DBconnector(driver, dbhost, dbName, userName,
            pasword);
      // nawiazanie polaczenia z baza
      connector.updateDB(xmlFiles); // update bazy danych wg plikow xml

      // ////////////////////wyszukiwanie polaczen z wydrukiem wszystkiego...
      Time time = new Time(3, 25, 0);
      ArrayList<ArrayList<LineStop>> sResult = connector
            .findCourse(2, connector.getPrzystId("krzyki"), connector
                  .getPrzystId("eureka"), 1);
      ...
      for (int i = 0; i < sResult.size(); i++)
      {
         for (int k = 0; k < sResult.get(i).size(); k++)
         {
            System.out.print("("
                  + connector
                        .getLiniaNazwa(sResult.get(i).get(k).getLinia_id())
                  + ")");
            System.out.print("["
